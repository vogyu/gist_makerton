...
# 머신러닝 전처리 시작
# 모델 위치
model_filename = 'C:\\Users\\dksgm\\Downloads\\converted_keras\\keras_model.h5'
...
def search_max(p):
    ...
    if maxNum ==0 or maxNum ==1 or maxNum ==2 or maxNum ==4:
        pet_kind = 2 # YES-LABEL NO_COLOR
        pet_str = "no label clear PET"
    elif maxNum == 5 or maxNum ==6:
        pet_kind = 1 # NO-LABEL NO_COLOR
        pet_str = "label  clear PET"
    elif maxNum == 3:
        pet_kind = 3 # YES&NO-LABEL YES_COLOR
        pet_str = "color PET"
    return pet_kind;
...
def camfunc():
    ...
    while (True):
        ...
        if cam_on_flag == 5:
            ...
            max_flag = search_max(prediction)

            if max_flag >0: # NO-LABEL NO_COLOR
                cap.release()
                cv2.destroyAllWindows()
                SER.write([max_flag])
                cam_on_flag = 0
                message = (str(pet_str))  # message를 입력해서 보내줌
